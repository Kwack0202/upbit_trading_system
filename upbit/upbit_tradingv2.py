...
class Upbit_trading_system(QAxWidget):
    def __init__(self):
        ...
        #### 1. 종목 탐색 변수 지정 ####
        self.volume = 30000 * 1000000 # ~~~백만
        self.lower_change_rate = -0.03
        self.upper_change_rate = 0.10
        
        self.targeting_timer = 3 # 일정 시간 매수 신호가 포착되지 않을 경우 종목 변경을 위한 타이머
                    
        #### 2. 거래에서 사용하기 위한 시간봉 단위 및 추세 판단 기간 설정 ####
        self.time_frame = 5 # 5, 10, 15, 30, 60 
        self.time_sequence = None
        
        self.long_period = 24  # 예: 1440 (24시간)
        self.medium_period = 12  # 예: 720 (12시간)
        self.short_period = 6  # 예: 360 (6시간)
        
        
        #### 3. 손절 및 익절 기준 변수 설정 ####
        self.stop_loss = -7.0  # 손절 기준
        self.take_profit = 6.0  # 익절 기준
        self.take_profit_uptrend = 10.0  # 익절 기준(상승장)
        
        self.stop_loss_overrated = -3.0 # 고평가 손절 기준
        self.take_profit_overrated = 2.5 # 고평가 익절 기준
        self.take_profit_overrated_uptrend = 4.5 # 고평가 익절 기준(상승장)

        
        #### 4. 포지션 진입 및 청산에 사용할 기준 변수 ####
        # 1차 포지션 기준 (매수 기준만 존재)
        self.first_buy_rsi_fanic = 20 # 매수 기준(패닉셀)
        self.first_buy_rsi = 35 # 매수 기준 (횡보장, 상승장)
    ...
    def get_account_info(self):
        ...
        try:
            ...
            print("+-------------------------------------------------------------------------------+")
            print(f"|    손절 및 익절 조건    | 손절가: {self.stop_loss}% | 익절가: {self.take_profit}% | 익절가(상승장): {self.take_profit_uptrend}% |")
            print("+-------------------------------------------------------------------------------+")

                        
            print("+----------------------------------------------------------------------------------+")
            print(f"| 손절 및 익절 조건 (고평가) | 손절가: {self.stop_loss_overrated}% | 익절가: {self.take_profit_overrated}% | 익절가(상승장): {self.take_profit_overrated_uptrend}% |")
            print("+----------------------------------------------------------------------------------+")
    ...
    def select_target_ticker(self):
        ...
        print(f"조건2: 등락률 {self.lower_change_rate * 100}% 이상, {self.upper_change_rate * 100}% 이하")
        ...
        for coin in coin_info:
            ...
            if acc_trade_price_24h >= self.volume and (signed_change_rate <= self.upper_change_rate and signed_change_rate >= self.lower_change_rate):
        ...
        if filtered_coin_names:
            ...
            # 최근 10일 간의 1일봉 데이터를 가져와서 하루만에 30% 이상 등락된 경우를 체크
            ohlcv = pyupbit.get_ohlcv(self.target_ticker, "day1", count=10)
            
            # 30% 이상 등락된 날이 있는지 확인
            for i in range(1, len(ohlcv)):
                previous_close = ohlcv.iloc[i-1]['close']
                current_close = ohlcv.iloc[i]['close']
                price_change_rate = abs((current_close - previous_close) / previous_close) * 100
                
                if price_change_rate >= 30:
                    print(f"\n{self.target_ticker}는 최근 10일 사이에 하루만에 30% 이상 등락했습니다. 새로운 종목을 선정합니다.\n")
            ...
            print(f"\n조건을 통과한 코인의 수: {len(filtered_coin_names)} | Target ticker로 선정된 코인: {self.target_ticker}\n")
